...
export function isRemoteSelection(sel: string | null | undefined): sel is string {
  return !!sel && sel.startsWith("remote:");
}

export function normalizeSelection(sel: string | null | undefined, fallback = "model:gemma3:1b") {
  if (!sel) return fallback;
  if (isAgentSelection(sel) || isModelSelection(sel) || isRemoteSelection(sel)) return sel;
